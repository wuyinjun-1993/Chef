...
source_file="./hyper_${dataset_name}1"
...
python_file_list=(full_pipeline_suggest_labels full_pipeline_suggest_labels  full_pipeline full_pipeline_o2u full_pipeline_active_learning)
multi_strategy_list=(false true false false false)
suffix_list=(sl0 sl infl o2u ac)
clear_hist_list=(true false false false true)
default_strategy_list=(3 0 0 0 0)
same_y_diff_list=(true false false false false)
cmd


python_file_list=(full_pipeline_suggest_labels full_pipeline_suggest_labels  full_pipeline full_pipeline_o2u full_pipeline_active_learning)
multi_strategy_list=(true false false false false)
suffix_list=(sl sl0 infl o2u ac)
clear_hist_list=(true false false false true)
default_strategy_list=(0 3 0 0 0)
same_y_diff_list=(false true false false false)
...
do
	...
	for k in "${!python_file_list[@]}"; do
		...
		curr_y_dff=${same_y_diff_list[$k]}

		echo "${k} ${curr_python_file} ${multi_strategy}"
#<<cmd
        	echo "bash run_full_pipeline_suggest_label_repeat_all.sh ${hist_period}  ${1} $2 ${remove_count} ${repeat_num} $lr ${l2_decay} ${epochs} ${batch_size} ${curr_clear_hist} ${i} ${isGPU} ${GPUID} ${derived_lr} 1 ${curr_python_file} ${multi_strategy} ${curr_suffix} ${default_strategy} ${source_file} ${model} ${curr_y_dff}"

	        bash run_full_pipeline_suggest_label_repeat_all.sh ${hist_period}  ${1} $2 ${remove_count} ${repeat_num} $lr ${l2_decay} ${epochs} ${batch_size} ${curr_clear_hist} ${i} ${isGPU} ${GPUID} ${derived_lr} 1 ${curr_python_file} ${multi_strategy} ${curr_suffix} ${default_strategy} ${source_file} ${model} ${curr_y_dff}
<<cmd
		if [ "${dataset_name}" == 'retina' ];
	        then
