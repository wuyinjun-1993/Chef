...
source_file="./hyper_${dataset_name}"
...
python_file_list=(full_pipeline_suggest_labels full_pipeline_tars)
...
#<<cmd
        	echo "bash run_full_pipeline_suggest_label_repeat_all.sh ${hist_period}  ${1} $2 ${remove_count} ${repeat_num} $lr ${l2_decay} ${epochs} ${batch_size} ${curr_clear_hist} ${i} ${isGPU} ${GPUID} ${derived_lr} ${regular_rate} ${curr_python_file} ${multi_strategy} ${curr_suffix} ${default_strategy} ${source_file} ${model}"

	        bash run_full_pipeline_suggest_label_repeat_all.sh ${hist_period}  ${1} $2 ${remove_count} ${repeat_num} $lr ${l2_decay} ${epochs} ${batch_size} ${curr_clear_hist} ${i} ${isGPU} ${GPUID} ${derived_lr} ${regular_rate} ${curr_python_file} ${multi_strategy} ${curr_suffix} ${default_strategy} ${source_file} ${model}
<<cmd
		if [ "${dataset_name}" == 'retina' ];
	        then
