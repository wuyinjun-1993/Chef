...
#mkdir ${dataset_dir}/prev_output

#cp ${dataset_dir}/*.txt ${dataset_dir}/prev_output/


curr_out_dir='curr_output'

mkdir ${dataset_dir}/${curr_out_dir}
...
source_file="./hyper_${dataset_name}${tag}_incre"
...
suffix_list=(sl)
clear_hist_list=(true)
default_strategy_list=(3)
#run_incremental_list=(true)
...
#		run_incremental=${run_incremental_list[$k]}

		echo "${k} ${curr_python_file} ${multi_strategy}"
#<<cmd
        	echo "bash run_full_pipeline_suggest_label_repeat_all_compare_incre.sh ${hist_period}  ${1} $2 ${remove_count} ${repeat_num} $lr ${l2_decay} ${epochs} ${batch_size} ${curr_clear_hist} ${i} ${isGPU} ${GPUID} ${derived_lr} ${regular_rate} ${curr_python_file} ${multi_strategy} ${curr_suffix} ${default_strategy} ${source_file} ${model} ${curr_out_dir} ${is_tars}"

	        bash run_full_pipeline_suggest_label_repeat_all_compare_incre.sh ${hist_period}  ${1} $2 ${remove_count} ${repeat_num} $lr ${l2_decay} ${epochs} ${batch_size} ${curr_clear_hist} ${i} ${isGPU} ${GPUID} ${derived_lr} ${regular_rate} ${curr_python_file} ${multi_strategy} ${curr_suffix} ${default_strategy} ${source_file} ${model} ${curr_out_dir} ${is_tars}
<<cmd
		if [ "${dataset_name}" == 'retina' ];
	        then
