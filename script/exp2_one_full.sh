...
python_file_list=(full_pipeline_suggest_labels full_pipeline_suggest_labels full_pipeline full_pipeline_o2u full_pipeline_active_learning full_pipeline_active_learning_entropy)
multi_strategy_list=(true false false false false false)
suffix_list=(sl${source_file_suffix} sl0${source_file_suffix} infl${source_file_suffix} o2u${source_file_suffix} ac${source_file_suffix} ac2${source_file_suffix})
clear_hist_list=(true false false false false false)
default_strategy_list=(0 3 0 0 0 0)
y_diff_list=(false true false false false false)
...
#	echo "bash run_full_pipeline_suggest_label_repeat_all_neural.sh ${hist_period}  ${1} $2 ${remove_count} ${repeat_num} $lr ${l2_decay} ${epochs} ${batch_size} ${curr_clear_hist} ${i} ${isGPU} ${GPUID} ${derived_lr} ${regular_rate} ${curr_python_file} ${multi_strategy} ${curr_suffix} ${default_strategy} ${source_file}_neural ${model} false"
 ...
 #               bash run_full_pipeline_suggest_label_repeat_all_neural.sh ${hist_period}  ${1} $2 ${remove_count} ${repeat_num} $lr ${l2_decay} ${epochs} ${batch_size} ${curr_clear_hist} ${i} ${isGPU} ${GPUID} ${derived_lr} ${regular_rate} ${curr_python_file} ${multi_strategy} ${curr_suffix} ${default_strategy} ${source_file}_neural ${model} false
	...
	for k in "${!python_file_list[@]}"; do
		...
		curr_y_diff=${y_diff_list[$k]}

		echo "${k} ${curr_python_file} ${multi_strategy} ${curr_y_diff}"
#<<cmd
        	echo "bash run_full_pipeline_suggest_label_repeat_all.sh ${hist_period}  ${1} $2 ${remove_count} ${repeat_num} $lr ${l2_decay} ${epochs} ${batch_size} ${curr_clear_hist} ${i} ${isGPU} ${GPUID} ${derived_lr} ${regular_rate} ${curr_python_file} ${multi_strategy} ${curr_suffix} ${default_strategy} ${source_file} ${model} ${curr_y_diff} ${is_tars}"

	        bash run_full_pipeline_suggest_label_repeat_all.sh ${hist_period}  ${1} $2 ${remove_count} ${repeat_num} $lr ${l2_decay} ${epochs} ${batch_size} ${curr_clear_hist} ${i} ${isGPU} ${GPUID} ${derived_lr} ${regular_rate} ${curr_python_file} ${multi_strategy} ${curr_suffix} ${default_strategy} ${source_file} ${model} ${curr_y_diff} ${is_tars}
<<cmd
		if [ "${dataset_name}" == 'retina' ];
	        then
