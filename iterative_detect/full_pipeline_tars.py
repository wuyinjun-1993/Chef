...
def pre_compute_influence_order_history(args,w_list, grad_list, model, training_dataset, loss_func, optimizer, random_ids_all_epochs, batch_size, regularization_rate, model_param, is_GPU, device, num_class, model_param_per_epoch_list, stopped_epoch, hist_period, dir_prefix, full_out_dir):
...
#         torch.save(max_mu_list, full_out_dir + '/max_mu_list_tensor_sl_' + str(iter_count))
        ...
        sample_wise_gradient = compute_sample_class_wise_gradient(training_dataset, curr_grad.view(-1).shape[0],model,optimizer,num_class, is_GPU, device)
...
#         origin_sorted_train_ids0 = remaining_ids0[sorted_train_ids_0] 
    ...
    #         print(p, Y_class)
        ...
        remaining_train_prob_case2 = train_prob_case2[remaining_ids]
        ...
        if if_dnn:
            influences1, full_grad_tensors, ordered_list, origin_sorted_train_ids0, s_test_vec = origin_compute_sample_class_wise_gradients2(args, args.num_class, model, optimizer, loss_func, full_out_dir, remaining_dataset_full, validation_dataset, dataset_test, curr_w_list, curr_grad_list, args.GPU, args.device, args.wd,args.bz, full_training_noisy_dataset.data.shape[0], full_training_noisy_dataset, Y_difference = Y_difference[:,remaining_ids], derived_lr = derived_lr, regular_rate = regular_rate, r_weight = r_weight, train_prob_case2 = remaining_train_prob_case2, train_r_weight = train_r_weight)
        
        else:
            
            '''args, num_class, model, optimizer, loss_func, full_out_dir, dataset_train, valid_dataset, test_dataset, curr_w_list, curr_grad_list, is_GPU, device, regularization_coeff, batch_size, full_data_size, full_dataset_train, s_test_vec_tensor = None, Y_difference = None, derived_lr = 0.0002, regular_rate = 0.1, r_weight = None, train_prob_case2 = None'''
        ...
        #torch.floor(origin_sorted_train_ids0/remaining_ids.shape[0])
        ...
        suggested_updated_labels = consolidate_label_list_tensor[origin_sorted_train_ids]
...
def model_training_main(full_out_dir, model, optimizer, random_ids_multi_super_iterations, w_list, grad_list, full_training_noisy_dataset, updated_labels, args, validation_dataset, dataset_test, is_incremental, iter_count, ids_with_changed_ids, ids_with_unchanged_ids, full_existing_labeled_id_tensor, start = False, regularization_prob_samples = 0.1, r_weight = None):
    ...
    if args.incremental and iter_count - 1 == last_iter:
        ...
        w_diff_remove_1, c2, mu, all_epochs = pre_compute_influence_order_history(args,w_list, grad_list, model, updated_dataset, model.soft_loss_function_reduce, optimizer, random_ids_multi_super_iterations, args.bz, args.wd, model_param, args.GPU, args.device, args.num_class, model_param_per_epoch_list, stopped_epoch, args.hist_period, full_out_dir + '/max_mu_list_tensor_' + args.suffix + '_' + str(iter_count), full_out_dir)
...
#         if continue_labeling:
        ...
        updated_labels, full_existing_labeled_id_tensor, ids_with_changed_ids, ids_with_unchanged_ids = select_samples_influence_function_main(args, full_training_noisy_dataset, full_training_origin_labels, validation_dataset, dataset_test, model, full_out_dir, is_incremental, iter_count + 1, full_existing_labeled_id_tensor,w_list, grad_list, derive_probab_labels = False, train_annotated_label_tensor = train_annotated_label_tensor, derived_lr = args.lr, regular_rate = regularization_prob_samples, r_weight = r_weight, suffix = suffix)
