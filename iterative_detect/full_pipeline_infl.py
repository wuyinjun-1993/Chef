...
#
# def pre_compute_influence_order_history(args,w_list, grad_list, model, training_dataset, r_weight, loss_func, optimizer, random_ids_all_epochs, batch_size, regularization_rate, model_param, is_GPU, device, num_class, model_param_per_epoch_list, stopped_epoch, hist_period, dir_prefix, dir_prefix2, full_out_dir):
#
#     mu, c2, grad_bound_list, mu_bound_list = compute_lower_bound_hessian_w(grad_list, w_list, model, optimizer, loss_func, random_ids_all_epochs, training_dataset, r_weight, batch_size, regularization_rate, is_GPU, device)
#
...
#
# #     num_para_count = 
#     curr_grad = get_all_vectorized_parameters1(grad_list[0])
#     for k in range(len(model_param_per_epoch_list)):
#
#         if not (k - stopped_epoch)%hist_period == 0:
#             continue 
#
#         model_param = model_param_per_epoch_list[k]
#
...
#
#
#         max_sample_mu_list = compute_max_mu_sample_wise(training_dataset, model, model_param, loss_func, optimizer, is_GPU, device)
#
# #         torch.save(max_mu_list, full_out_dir + '/max_mu_list_tensor_sl_' + str(iter_count))
#
#         set_model_parameters(model, model_param, device)
#
#         sample_wise_gradient, full_sample_wise_gradient = compute_sample_class_wise_gradient(args.wd, training_dataset, curr_grad.view(-1).shape[0],model,optimizer,num_class, model.soft_loss_function_reduce, is_GPU, device)
#
#         torch.save(torch.tensor(max_mu_list), dir_prefix + '_epoch_' + str(k)+ args.resolve_conflict_str)
#
#         torch.save(torch.tensor(max_sample_mu_list), dir_prefix2 + '_epoch_' + str(k)+ args.resolve_conflict_str)
#
#         torch.save(sample_wise_gradient, full_out_dir + '/pre_compute_sample_wise_grad_epoch_' + str(k)+ args.resolve_conflict_str)
#
#         torch.save(full_sample_wise_gradient, full_out_dir + '/pre_compute_full_sample_wise_grad_epoch_' + str(k)+ args.resolve_conflict_str)
# #         torch.save(torch.tensor(max_mu_list), dir_prefix + '_epoch_' + str(k))
#
#         all_epochs.append(k)
#
#     t2 = time.time()
...
#
# #     torch.save(torch.tensor(all), dir_prefix + '_epoch_' + str(k))
...
#
#     return w_diff_remove_1_2, c2, mu, all_epochs
#
#
#
#
...
#
#             # if not regularization_prob_samples == 0:
...
#
#         if if_dnn:
#             influences1, full_grad_tensors, ordered_list, origin_sorted_train_ids0, s_test_vec = origin_compute_sample_class_wise_gradients2(args, args.num_class, model, optimizer, loss_func, full_out_dir, remaining_dataset_full, validation_dataset, dataset_test, curr_w_list, curr_grad_list, args.GPU, args.device, args.wd,args.bz, full_training_noisy_dataset.data.shape[0], training_dataset, Y_difference = Y_difference[:,remaining_ids], regular_rate = regular_rate, r_weight = r_weight, train_r_weight = train_r_weight)
#
#         else:
#             influences1, full_grad_tensors, ordered_list, origin_sorted_train_ids0, s_test_vec = origin_compute_sample_class_wise_gradients(args, args.num_class, model, optimizer, loss_func, full_out_dir, remaining_dataset_full, validation_dataset, dataset_test, curr_w_list, curr_grad_list, args.GPU, args.device, args.wd,args.bz, full_training_noisy_dataset.data.shape[0], training_dataset, Y_difference = Y_difference[:,remaining_ids], regular_rate = regular_rate, r_weight = r_weight, train_r_weight = train_r_weight)
#
...
#
#         max_sample_mu_list_tensor = torch.load(full_out_dir + '/max_sample_mu_list_tensor_' + suffix + '_' + str(last_iter + 1) + '_epoch_' + str(selected_epoch.item())+ args.resolve_conflict_str)
#
...
#
#         prev_full_sample_wise_grad_tensors = torch.load(full_out_dir + '/pre_compute_full_sample_wise_grad_epoch_' + str(selected_epoch.item()) + args.resolve_conflict_str)
#
...
#
#         set_model_parameters(model, origin_model_param, args.device)
#
#         loss_func = model.soft_loss_function_reduce
#
#         full_grad_tensors0, full_grad_tensors1 = compute_sample_wise_gradient(args, args.num_class, model, optimizer, loss_func, full_out_dir, full_training_noisy_dataset, validation_dataset, dataset_test, curr_w_list, curr_grad_list, args.GPU, args.device, args.wd,args.bz, full_training_noisy_dataset.data.shape[0], full_training_noisy_dataset, s_test_vec_tensor = None, regular_rate = regular_rate, r_weight = r_weight)
#
#         prev_full_sample_wise_grad_tensors = full_grad_tensors1
#
...
#             Y_difference = torch.ones([args.num_class, full_training_noisy_dataset.data.shape[0], args.num_class], dtype = torch.double)
#
#         '''X, Y, batch_size, is_GPU, device, full_grad_tensors, y_difference, full_data_size, full_grad_tensors1, full_grad_tensors_multi_class, regular_rate = 0.1, regularization_term = 0, train_grad = None'''
#         # curr_w_tensor = get_all_vectorized_parameters1(list(model.parameters()), args.device)
#         #
#         # regularization_term = args.wd*curr_w_tensor    
#
#         origin_class_wise_grad_tensor = None#pre_compute_class_grad(full_training_noisy_dataset.data, full_training_noisy_dataset.labels, args.bz, args.GPU, args.device, prev_full_grad_tensors, Y_difference, full_training_noisy_dataset.data.shape[0], full_grad_tensors1 = prev_full_sample_wise_grad_tensors, full_grad_tensors_multi_class = prev_full_grad_tensors_multi_class, regular_rate = args.regular_rate, regularization_term = None)
#
#
#         if args.GPU:
#             torch.cuda.synchronize(device = args.device)
#         t1 = time.time()
#
#         '''                                                                                args, num_class, model, optimizer, loss_func, full_out_dir, dataset_train, valid_dataset, test_dataset, is_GPU, device, regularization_coeff, batch_size, full_data_size, full_dataset_train, model_diff, origin_sample_wise_grad, origin_sample_wise_grad_full, max_mu_list_tensor, max_sample_mu_list_tensor, full_removed_id_tensor_0, full_existing_labeled_id_tensor, full_origin_influence_list_tensor, prev_s_test_vec, y_difference, regular_rate = 0.0, r_weight = None'''
#         final_remaining_ids, s_test_vec_tensor, full_influence_list_tensor0 = obtain_remaining_ids_incremental_class_wise2(args, args.num_class, model, optimizer, loss_func, full_out_dir, full_training_noisy_dataset, validation_dataset, dataset_test, args.GPU, args.device, args.wd, args.bz, full_training_noisy_dataset.data.shape[0], full_training_noisy_dataset, delta_model_param, prev_full_grad_tensors, prev_full_grad_tensors_multi_class, prev_full_sample_wise_grad_tensors, max_mu_list_tensor, max_sample_mu_list_tensor, full_removed_id_tensor_0, full_existing_labeled_id_tensor, origin_influences1, prev_s_test_vec, Y_difference, regular_rate = regular_rate, r_weight = r_weight, origin_class_wise_grad_tensor = origin_class_wise_grad_tensor)
#
...
#         influences1_2, full_grad_tensors_2, ordered_list_2, origin_sorted_train_ids_2_0, s_test_vec_2 = origin_compute_sample_class_wise_gradients(args, args.num_class, model, optimizer, loss_func, full_out_dir, remaining_dataset, validation_dataset, dataset_test, curr_w_list, curr_grad_list, args.GPU, args.device, args.wd,args.bz, full_training_noisy_dataset.data.shape[0], full_training_noisy_dataset, s_test_vec_tensor = s_test_vec_tensor, Y_difference = Y_difference[:,final_remaining_ids], regular_rate = regular_rate, r_weight = r_weight)
#
...
# #         
# #         optimizer0 = model0.get_optimizer(args.derived_lr, args.wd)
# #         
...
#
#         # final_remaining_ids_0, s_test_vec_tensor, full_influence_list_tensor0 = obtain_remaining_ids_incremental_class_wise2(args, args.num_class, model, optimizer, loss_func, full_out_dir, full_training_noisy_dataset, validation_dataset, dataset_test, args.GPU, args.device, args.wd, args.bz, full_training_noisy_dataset.data.shape[0], full_training_noisy_dataset, delta_model_param, prev_full_grad_tensors, prev_full_grad_tensors_multi_class, prev_full_sample_wise_grad_tensors, max_mu_list_tensor, max_sample_mu_list_tensor, full_removed_id_tensor_0, full_existing_labeled_id_tensor, origin_influences1, prev_s_test_vec, Y_difference, regular_rate = regular_rate, r_weight = r_weight, origin_class_wise_grad_tensor = origin_class_wise_grad_tensor, s_test_vec_tensor = s_test_vec_tensor)
#
...
#
#
#     args.removed_count = min(remaining_ids.shape[0], args.removed_count)
#
...
#
# def print_gpu_utilization(GPU_mem_usage_list, GPU_utilization_list):
#     avg_GPU_mem_usage = np.average(np.array(GPU_mem_usage_list))
#     print(gpu_mem_prefix, avg_GPU_mem_usage)
#
#     if GPU_utilization_list is not None:
#         avg_GPU_util_usage = np.average(np.array(GPU_utilization_list))
#
#         print(gpu_utilization_prefix, avg_GPU_util_usage)
#
#
#
#
...
#                 stopped_epoch = torch.load(full_out_dir + '/stopped_epoch_' + args.suffix + '_v' + str(iter_count-1) + '_incre' + args.resolve_conflict_str)
#             else:
#                 stopped_epoch = torch.load(full_out_dir + '/stopped_epoch_' + args.suffix + '_v' + str(iter_count-1) + '_incre')
#
...
#     sorted_train_ids = torch.load(full_out_dir + '/' + args.model + '_influence_removed_ids_' + args.suffix + '_v' + str(iter_count) + incremental_suffix)
#
#     ordered_list = torch.load(full_out_dir + '/' + args.model + '_influence_removed_ids_weight_' + args.suffix + '_v' + str(iter_count) + incremental_suffix)
#
#     influences1 = torch.load(full_out_dir + '/' + args.model + '_influence_' + args.suffix + '_v' + str(iter_count) + incremental_suffix)    
#
#     exp_sorted_train_ids = torch.load(full_out_dir + '/' + args.model + '_influence_removed_ids_' + args.suffix + '_v' + str(iter_count))
#
#     exp_ordered_list = torch.load(full_out_dir + '/' + args.model + '_influence_removed_ids_weight_' + args.suffix + '_v' + str(iter_count))
#
#     influences2 = torch.load(full_out_dir + '/' + args.model + '_influence_' + args.suffix + '_v' + str(iter_count))
#
#     return influences2,exp_ordered_list,exp_sorted_train_ids,influences1,ordered_list,sorted_train_ids 
#
#
# #     print('sub time2::', t3 - t4)
# def load_retraining_history_info(origin_full_training_noisy_dataset, iter_count, full_out_dir, args):
...
#
#     if type(full_training_noisy_dataset) is torch.Tensor:
#         full_training_noisy_dataset = MyDataset(origin_full_training_noisy_dataset.data, full_training_noisy_dataset)
#
...
#
# def load_retraining_history_info0(origin_full_training_noisy_dataset, iter_count, full_out_dir, args):
...
#
#
# def load_incremental_history_info(origin_full_training_noisy_dataset, iter_count, full_out_dir, args):
...
#         full_training_noisy_dataset = MyDataset(origin_full_training_noisy_dataset.data, full_training_noisy_dataset)
#
#     model = torch.load(full_out_dir + '/model_' + args.suffix + '_v' + str(iter_count) + incremental_suffix + args.resolve_conflict_str, map_location=torch.device('cpu'))
#
#     model = model.to(args.device)
#
#     random_ids_multi_super_iterations = torch.load(full_out_dir + '/random_ids_multi_super_iterations')
...
#     return full_existing_labeled_id_tensor, full_training_noisy_dataset, model, random_ids_multi_super_iterations, w_list, grad_list
#
# def load_incremental_history_info0(origin_full_training_noisy_dataset, iter_count, full_out_dir, args):
...
#
#
# def load_history_info(full_training_noisy_dataset, is_incremental, iter_count, full_out_dir, args):
#
...
#         if len(args.resolve_conflict_str) == 0:
#             return load_retraining_history_info0(full_training_noisy_dataset, iter_count, full_out_dir, args)
#         else:
#             return load_retraining_history_info(full_training_noisy_dataset, iter_count, full_out_dir, args)
#
#
#
#
...
#
#         if len(args.resolve_conflict_str) == 0:
#             return load_incremental_history_info0(full_training_noisy_dataset, iter_count, full_out_dir, args)
#         else:
#             return load_incremental_history_info(full_training_noisy_dataset, iter_count, full_out_dir, args)
#
#
#
#
...
#     size = None
#
#     continue_labeling = args.incremental
#
...
#             random_ids_multi_super_iterations = torch.load(full_out_dir + '/random_ids_multi_super_iterations')
#
#             stopped_epoch = torch.load(full_out_dir + '/stopped_epoch_initial')
#
#         else:
#
...
#
#             else:
#                 origin_labeled_tensor = torch.Tensor()
#
#
...
#
#             torch.save(stopped_epoch, full_out_dir + '/stopped_epoch_initial')
#
#
...
#         if if_dnn:
#
#             valid_model_dataset(model, valid_origin_dataset, None, args.bz, 'init validation', args.GPU, args.device, f1=False)
#
#             valid_model_dataset(model, test_origin_dataset, None, args.bz, 'init test', args.GPU, args.device, f1=False)
#
#             valid_model_dataset(model, valid_origin_dataset, None, args.bz, 'init validation F1', args.GPU, args.device, f1=True)
#
#             valid_model_dataset(model, test_origin_dataset, None, args.bz, 'init test F1', args.GPU, args.device, f1=True)
#
#         else:
...
#
#         if if_dnn:
#             torch.save(train_origin_dataset.labels, full_out_dir + '/full_training_noisy_dataset_' + suffix + '_v0')
#         else:
...
#         args.resolve_conflict_str = ''
#
#         if args.resolve_conflict is not None:
#             args.resolve_conflict_str = resolve_conflict_tag + '_' + str(args.resolve_conflict)
#
#
#         full_existing_labeled_id_tensor, full_training_noisy_dataset, model, random_ids_multi_super_iterations, w_list, grad_list= load_history_info(full_training_noisy_dataset, load_incremental, iter_count, full_out_dir, args)
#
#         model = model.to(args.device)
#
#         if if_dnn:
...
#
#
#         updated_labels, full_existing_labeled_id_tensor, ids_with_changed_ids, ids_with_unchanged_ids = select_samples_influence_function_main(args, full_training_noisy_dataset, full_training_origin_labels, validation_dataset, dataset_test, model, full_out_dir, is_incremental, iter_count + 1, full_existing_labeled_id_tensor,w_list, grad_list, derive_probab_labels = False, train_annotated_label_tensor = train_annotated_label_tensor, regular_rate = regularization_prob_samples, r_weight = r_weight, suffix = suffix)
#
#
# #         if continue_labeling:
#
...
#         if not is_incremental:
#             if if_dnn:
#                 torch.save(full_training_noisy_dataset.labels, full_out_dir + '/full_training_noisy_dataset_' + suffix + '_v' + str(iter_count+1) + args.resolve_conflict_str)
#             else:
#                 torch.save(full_training_noisy_dataset, full_out_dir + '/full_training_noisy_dataset_' + suffix + '_v' + str(iter_count+1) + args.resolve_conflict_str)
#
#             torch.save(full_existing_labeled_id_tensor, full_out_dir + '/full_existing_labeled_id_tensor_' + suffix + '_v' + str(iter_count+1) + args.resolve_conflict_str)
#
#         else:
#             if if_dnn:
#                 torch.save(full_training_noisy_dataset.labels, full_out_dir + '/full_training_noisy_dataset_' + suffix + '_v' + str(iter_count+1) + incremental_suffix + args.resolve_conflict_str)
#             else:
#                 torch.save(full_training_noisy_dataset, full_out_dir + '/full_training_noisy_dataset_' + suffix + '_v' + str(iter_count+1) + incremental_suffix + args.resolve_conflict_str)
#
# #             torch.save(model, full_out_dir + '/model_v' + str(iter_count+1) + incremental_suffix)
#
#             torch.save(full_existing_labeled_id_tensor, full_out_dir + '/full_existing_labeled_id_tensor_' + suffix + '_v' + str(iter_count+1) + incremental_suffix + args.resolve_conflict_str)
#
#         if continue_labeling or args.continue_labeling:
#
...
#     num_para_count = 
    ...
    for k in range(len(model_param_per_epoch_list)):
        ...
        max_sample_mu_list = compute_max_mu_sample_wise(args, training_dataset, model, model_param, loss_func, optimizer, is_GPU, device)
...
#         loss_gap2 = compute_loss_valid_dataset_grad(validation_dataset, model, loss_func, optimizer, w_diff_remove_1, get_model_para_list(origin_model))
        ...
        origin_class_wise_grad_tensor = pre_compute_class_grad(full_training_noisy_dataset.data, full_training_noisy_dataset.labels, args.bz, args.GPU, args.device, prev_full_grad_tensors, Y_difference, full_training_noisy_dataset.data.shape[0], full_grad_tensors1 = prev_full_sample_wise_grad_tensors, full_grad_tensors_multi_class = prev_full_grad_tensors_multi_class, regular_rate = args.regular_rate, regularization_term = None)
...
#         t3 = time.time()
        ...
        remaining_dataset = models.MyDataset(full_training_noisy_dataset.data[final_remaining_ids], full_training_noisy_dataset.labels[final_remaining_ids])
        influences1_2, full_grad_tensors_2, ordered_list_2, origin_sorted_train_ids_2_0, s_test_vec_2 = origin_compute_sample_class_wise_gradients(args, args.num_class, model, optimizer, loss_func, full_out_dir, remaining_dataset, validation_dataset, dataset_test, curr_w_list, curr_grad_list, args.GPU, args.device, args.wd,args.bz, full_training_noisy_dataset.data.shape[0], full_training_noisy_dataset, s_test_vec_tensor = s_test_vec_tensor, Y_difference = Y_difference[:,final_remaining_ids], regular_rate = regular_rate, r_weight = r_weight, train_r_weight = train_r_weight)
...
#         influences1, full_grad_tensors, ordered_list, sorted_train_ids, s_test_vec = origin_compute_sample_wise_gradients(model, optimizer, loss_func, full_out_dir, remaining_dataset_full, validation_dataset, dataset_test, curr_w_list, curr_grad_list, args.GPU, args.device, args.wd,args.bz, full_training_noisy_dataset.data.shape[0], full_training_noisy_dataset)
        '''influences1, full_grad_tensors, ordered_list, origin_sorted_train_ids0, s_test_vec = origin_compute_sample_class_wise_gradients(args, args.num_class, model, optimizer, loss_func, full_out_dir, remaining_dataset_full, validation_dataset, dataset_test, curr_w_list, curr_grad_list, args.GPU, args.device, args.wd,args.bz, full_training_noisy_dataset.data.shape[0], training_dataset, Y_difference = Y_difference[:,remaining_ids], regular_rate = regular_rate, r_weight = r_weight, train_r_weight = train_r_weight)'''
        influences1, full_grad_tensors, ordered_list, origin_sorted_train_ids0, s_test_vec = origin_compute_sample_class_wise_gradients(args, args.num_class, model, optimizer, loss_func, full_out_dir, remaining_dataset_full, validation_dataset, dataset_test, curr_w_list, curr_grad_list, args.GPU, args.device, args.wd,args.bz, full_training_noisy_dataset.data.shape[0], full_training_noisy_dataset, Y_difference = Y_difference[:,remaining_ids], regular_rate = regular_rate, s_test_vec_tensor = s_test_vec_tensor, r_weight = r_weight, train_r_weight = train_r_weight)
...
def model_training_main(full_out_dir, model, optimizer, random_ids_multi_super_iterations, w_list, grad_list, full_training_noisy_dataset, updated_labels, args, validation_dataset, dataset_test, is_incremental, iter_count, ids_with_changed_ids, ids_with_unchanged_ids, full_existing_labeled_id_tensor, start = False, regularization_prob_samples = 0.1, r_weight = None):
    ...
    if args.incremental and iter_count - 1 == last_iter:
        ...
        updated_dataset = models.MyDataset(full_training_noisy_dataset.data, updated_labels)
        all_epochs = pre_compute_influence_order_history(args,w_list, grad_list, model, full_training_noisy_dataset, r_weight, model.soft_loss_function, optimizer, random_ids_multi_super_iterations, args.bz, args.wd, model_param, args.GPU, args.device, args.num_class, model_param_per_epoch_list, stopped_epoch, args.hist_period, full_out_dir + '/max_mu_list_tensor_' + args.suffix + '_' + str(iter_count),full_out_dir + '/max_sample_mu_list_tensor_' + args.suffix + '_' + str(iter_count), full_out_dir)
        # torch.save(w_diff_remove_1, full_out_dir + '/w_diff_remove_1_' + args.suffix + '_' + str(iter_count)+ args.resolve_conflict_str)
