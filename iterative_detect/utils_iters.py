...
#         
        ...
        # res_vec_list = get_devectorized_parameters(vectorized_w, full_shape_list, shape_list)
        #
        # curr_r_weights = torch.ones(curr_X.shape[0], dtype = curr_X.dtype)
        #
        # hessian_w_prod,_,_ = compute_gradient3(model, curr_X, curr_Y, loss_func, curr_r_weights, res_vec_list, optimizer, is_GPU, device, random_sampling = False, bz = 1, batch_ids = None)
...
def compute_single_min_mu(args, init_w, iterative_times, model, loss_func, optimizer, is_GPU, device, curr_X, curr_Y, full_shape_list, shape_list, total_shape_size, model_param, model_grad):
...
#         
        ...
        curr_train_r_weights = torch.ones(curr_X.shape[0], dtype = torch.double)
        
        hessian_w_prod = compute_conjugate_grad3(model, vectorized_w, curr_X, curr_Y, loss_func, optimizer, curr_train_r_weights, 0, is_GPU, device, exp_res = None, learning_rate = args.derived_lr_min_mu, random_sampling = True, bz = 1, running_iter = args.derived_epochs_min_mu, regularization_rate=args.derived_l2)
    ...
    hessian_w_prod = compute_conjugate_grad3(model, vectorized_w, curr_X, curr_Y, loss_func, optimizer, curr_train_r_weights, 0, is_GPU, device, exp_res = None, learning_rate = args.derived_lr, random_sampling = True, bz = args.derived_bz, running_iter = args.derived_epochs, regularization_rate=args.derived_l2)
...
def compute_single_max_mu2(init_w, iterative_times, model, loss_func, optimizer, is_GPU, device, curr_X, curr_Y, full_shape_list, shape_list, total_shape_size, model_param, model_grad):
...
#         for p in range(num_class):
        ...
        if isinstance(class_id, int):
            model_out.view(-1)[class_id].backward(retain_graph = True)
        else:
            if type(class_id) is tuple:
                torch.abs(model_out.view(-1)[class_id[0]] - model_out.view(-1)[class_id[1]]).backward(retain_graph = True)
...
#         curr_loss.backward()
        ...
        if type(class_id) is tuple:
            ...
            hessian_w_prod = compute_model_hessian_vec_prod(model_out.view(-1)[class_id[0]] - model_out.view(-1)[class_id[1]], model, get_devectorized_parameters(vectorized_w, full_shape_list, shape_list), False, device)
            ...
            hessian_w_prod0 = compute_model_hessian_vec_prod(model_out.view(-1)[class_id], model, get_devectorized_parameters(vectorized_w, full_shape_list, shape_list), False, device)
...
#     curr_loss = loss_func(model(curr_X), curr_Y)
    ...
    if isinstance(class_id, int):
        ...
        if type(class_id) is tuple:
            (model_out.view(-1)[class_id[0]] - model_out.view(-1)[class_id[1]]).backward(retain_graph = True)
    
    
    if type(class_id) is tuple:
            set_model_parameters(model, model_param, device)
            hessian_w_prod = compute_model_hessian_vec_prod(torch.abs(model_out.view(-1)[class_id[0]] - model_out.view(-1)[class_id[1]]), model, get_devectorized_parameters(vectorized_w, full_shape_list, shape_list), False, device)
...
def compute_single_max_mu_class_wise2(init_w, iterative_times, model, loss_func, optimizer, is_GPU, device, curr_X, curr_Y, full_shape_list, shape_list, total_shape_size, model_param, model_grad, class_id):
...
def compute_max_mu_sample_wise(args, dataset_train, model, model_param, loss_func, optimizer, is_GPU, device):
    ...
    init_w = torch.rand(vec_model_param.view(-1).shape[0], device = device, dtype = torch.double)
...
#         t3 = time.time()
        ...
        # min_u = compute_single_min_mu(args,init_w.clone(), iterative_times, model, loss_func, optimizer, is_GPU, device, curr_data, curr_label, full_shape_list, shape_list, total_shape_size, model_param, curr_grad)
        
        # compute_conjugate_grad3(model, valid_grad, training_dataset.data, training_dataset.labels, loss_func, optimizer, curr_train_r_weights, 0, is_GPU, device, exp_res = None, learning_rate = args.derived_lr, random_sampling = True, bz = args.derived_bz, running_iter = args.derived_epochs, regularization_rate=args.derived_l2)
        
        set_model_parameters(model, model_param, device)
        
        # max_u2 = compute_single_max_mu2(init_w.clone(), iterative_times, model, loss_func, optimizer, is_GPU, device, curr_data, curr_label, full_shape_list, shape_list, total_shape_size, model_param, curr_grad)
...
#         curr_loss = loss_func(, curr_label)
        ...
        #     optimizer.zero_grad()
        #
        #     torch.abs(model_out.view(-1)[0]-model_out.view(-1)[1]).backward()
        #
        #     curr_grad = get_vectorized_grads(model, device)
        #
        #     t3 = time.time()
        #
        #     max_u = compute_single_max_mu_class_wise(init_w.clone(), iterative_times, model, loss_func, optimizer, is_GPU, device, curr_data, curr_label, full_shape_list, shape_list, total_shape_size, model_param, curr_grad, (0,1))
...
def get_influence_sorted_class_wise_test(X, Y, batch_size, is_GPU, device, full_grad_tensors, y_difference, s_test_vec_tensor, full_data_size, sort = True, full_grad_tensors1 = None, full_grad_tensors_multi_class = None, regular_rate = 0.1, regularization_term = 0, train_prob_case2 = None, train_grad = None, origin_class_wise_grad_tensor = None):
    influence_list = []
...
#             '''batch_size, m'''
        ...
        all_curr_class_grad.append(curr_class_grad)
#         curr_class_grad = torch.sum(curr_y_difference.view(curr_y_difference.shape[0], curr_y_difference.shape[1], curr_y_difference.shape[2], 1)*curr_grad_tensor_full, 2)
#         torch.bmm(torch.transpose(curr_grad_tensor, 1,2))
...
#         print(influence_list_tensor[i] - influences2[i])
    ...
    if sort:
        ...
        return influence_list_tensor, all_curr_class_grad
...
#     influence_list_tensor0  = get_influence_sorted_class_wise(X[full_removed_id_tensor_0], Y[full_removed_id_tensor_0], batch_size, is_GPU, device, origin_sample_wise_grad[full_removed_id_tensor_0], y_difference[:,full_removed_id_tensor_0], s_test_vec_tensor, full_data_size, sort = False, full_grad_tensors_multi_class = origin_sample_wise_grad_full[:,full_removed_id_tensor_0])
    ...
    if origin_class_wise_grad_tensor is not None:
        ...
        # influence_list_tensor0_1, origin_class_wise_grad_tensor_exp  = get_influence_sorted_class_wise_test(X[full_removed_id_tensor_0], Y[full_removed_id_tensor_0], batch_size, is_GPU, device, origin_sample_wise_grad[full_removed_id_tensor_0], y_difference[:,full_removed_id_tensor_0], s_test_vec_tensor, full_data_size, sort = False, full_grad_tensors1 = origin_full_sample_wise_grad_tensors[full_removed_id_tensor_0], full_grad_tensors_multi_class = origin_sample_wise_grad_full[:,full_removed_id_tensor_0], regular_rate = regular_rate, regularization_term = None)
...
#     delta_influence_list_tensor = 0.5*(torch.norm((model_diff).view(-1) + (s_test_vec_tensor).view(-1))**2)/full_data_size*(torch.sum(max_mu_list_tensor.view(1,max_mu_list_tensor.shape[0], max_mu_list_tensor.shape[1])*torch.abs(y_difference), dim = -1))
    ...
    delta_influence_list_tensor_upper = 0.5*model_diff_s_vec_dot*((1-regular_rate)*max_sample_mu_list_tensor)
     
    delta_influence_list_tensor_lower = 0.5*model_diff_s_vec_dot*((1-regular_rate)*max_sample_mu_list_tensor)
    ...
    extra_terms = 0.5*model_diff_s_vec_dot2*((1-regular_rate)*max_sample_mu_list_tensor)
    
    extra_terms = extra_terms/full_data_size
    
    delta_influence_list_tensor_upper += extra_terms 
    
    delta_influence_list_tensor_upper = delta_influence_list_tensor_upper + 0.5*(model_diff_s_vec_dot2 - model_diff_s_vec_dot)*max_sample_mu_list_tensor*args.rescale_hessian/full_data_size
    
    delta_influence_list_tensor_lower = extra_terms - delta_influence_list_tensor_lower
    
    
    delta_influence_list_tensor_lower = delta_influence_list_tensor_lower  + 0.5*(model_diff_s_vec_dot2 + model_diff_s_vec_dot)*max_sample_mu_list_tensor*args.rescale_hessian/full_data_size
...
#     id_set1 = remaining_removed_id0[args.removed_count:][((full_influence_list_tensor0+delta_influence_list_tensor)[remaining_removed_id0[args.removed_count:]].view(-1)) > ((full_influence_list_tensor0 - delta_influence_list_tensor)[remaining_removed_id0[args.removed_count-1]].view(-1))]
    ...
    sorted_influence_list_tensor1,sorted_influence_sample_ids1 = torch.sort((full_influence_list_tensor0+delta_influence_list_tensor_lower)[:,remaining_removed_id0].view(-1), descending=True)
    
    lower_bound_value = sorted_influence_list_tensor1[args.removed_count]
    
    correct_ids = torch.nonzero((full_influence_list_tensor0+delta_influence_list_tensor_upper).view(-1) > lower_bound_value)%full_data_size
    
    final_remaining_ids = torch.tensor(list(set(correct_ids.view(-1).tolist()).intersection(set(remaining_removed_id0.view(-1).tolist()))))
    
    print('lower bound::', lower_bound_value, model_diff_s_vec_dot2, model_diff_s_vec_dot)
    
    print('max_sample mu::', max_sample_mu_list_tensor, delta_influence_list_tensor_upper, delta_influence_list_tensor_lower)
    
    print(full_influence_list_tensor0)
    
    print(torch.sort((full_influence_list_tensor0+delta_influence_list_tensor_upper).view(-1))[0][0:100], torch.sort((full_influence_list_tensor0+delta_influence_list_tensor_upper).view(-1))[0][-100:])
    
    print(torch.sort((full_influence_list_tensor0+delta_influence_list_tensor_lower).view(-1))[0][0:100], torch.sort((full_influence_list_tensor0+delta_influence_list_tensor_lower).view(-1))[0][-100:])
...
#     sorted_id_set1 = sorted_remaining_removed_id0[args.removed_count:][([sorted_remaining_removed_id0[args.removed_count:]].view(-1)) < ((full_influence_list_tensor0 + delta_influence_list_tensor)[sorted_remaining_removed_id0[args.removed_count-1]].view(-1))]
    ...
    final_remaining_ids = torch.tensor(list(set(final_remaining_ids.tolist()).union(set(sorted_remaining_removed_id0[0:args.removed_count].view(-1).tolist()))))
...
#             print(model_out.shape)
        ...
        loss = loss_func(model(curr_x), curr_y)
