...
def select_samples_influence_function_main(args, full_training_noisy_dataset, full_training_origin_labels, validation_dataset, dataset_test, model, full_out_dir, is_incremental, iter_count, full_existing_labeled_id_tensor, curr_w_list, curr_grad_list, derive_probab_labels=False, method = 'ac', average_y_change=0.001):
...
#         origin_sorted_train_ids0 = remaining_ids0[sorted_train_ids_0] 
        ...
        influences1, full_grad_tensors, ordered_list, sorted_train_ids = origin_compute_sample_wise_gradients_ac(model, optimizer, loss_func, full_out_dir, remaining_dataset_full, validation_dataset, dataset_test, curr_w_list, curr_grad_list, args.GPU, args.device, args.wd,args.bz, full_training_noisy_dataset.data.shape[0], full_training_noisy_dataset, average_y_change = average_y_change)
        ...
        if not is_incremental:
            ...
            torch.save(origin_sorted_train_ids, full_out_dir + '/' + args.model + '_' + method + '_removed_ids_v' + str(iter_count))
        
            torch.save(ordered_list, full_out_dir + '/' + args.model + '_' + method + '_removed_ids_weight_v' + str(iter_count))
            
            torch.save(final_influence_values, full_out_dir + '/' + args.model + '_' + method + '_v' + str(iter_count))
            
#             torch.save(s_test_vec, full_out_dir + '/' + args.model + '_' + method + '_s_test_vec_v' + str(iter_count))
            
            torch.save(all_full_grad_tensors, full_out_dir + '/' + args.model + '_' + method + '_sample_wise_grad' + str(iter_count))
            
        else:
            origin_sorted_train_ids0 = torch.load(full_out_dir + '/' + args.model + '_' + method + '_removed_ids_v' + str(iter_count))
            
            torch.save(origin_sorted_train_ids, full_out_dir + '/' + args.model + '_' + method + '_removed_ids_v' + str(iter_count) + incremental_suffix)
        
            torch.save(ordered_list, full_out_dir + '/' + args.model + '_' + method + '_removed_ids_weight_v' + str(iter_count) + incremental_suffix)
    
            torch.save(final_influence_values, full_out_dir + '/' + args.model + '_' + method + '_v' + str(iter_count) + incremental_suffix)
            
#             torch.save(s_test_vec, full_out_dir + '/' + args.model + '_' + method + '_s_test_vec_v' + str(iter_count) + incremental_suffix)
            
            torch.save(all_full_grad_tensors, full_out_dir + '/' + args.model + '_' + method + '_sample_wise_grad' + str(iter_count) + incremental_suffix)
    else:
...
#         full_existing_labeled_id_tensor_0, full_training_noisy_dataset_0, model_0, random_ids_multi_super_iterations_0, w_list_0, grad_list_0 = load_retraining_history_info(iter_count-1, full_out_dir, args)
        ...
        prev_full_grad_tensors = torch.load(full_out_dir + '/' + args.model + '_' + method + '_sample_wise_grad' + str(last_iter+1) + incremental_suffix)
...
#         full_existing_labeled_id_tensor_0 = torch.load(full_out_dir + '/full_existing_labeled_id_tensor_v' + str(last_iter) + incremental_suffix)
        ...
        origin_influences1 = torch.load(full_out_dir + '/' + args.model + '_' + method + '_v' + str(last_iter+1) + incremental_suffix)
...
#         prev_s_test_vec = torch.load(full_out_dir + '/' + args.model + '_' + method + '_s_test_vec_v' + str(last_iter + 1) + incremental_suffix)
...
#         if final_remaining_ids is not None:
        ...
        influences1_2, full_grad_tensors_2, ordered_list_2, sorted_train_ids_2 = origin_compute_sample_wise_gradients_ac(model, optimizer, loss_func, full_out_dir, remaining_dataset, validation_dataset, dataset_test, curr_w_list, curr_grad_list, args.GPU, args.device, args.wd,args.bz, full_training_noisy_dataset.data.shape[0], full_training_noisy_dataset, average_y_change=average_y_change)
...
#         
        ...
        print('calculate influence time 1::', t6 - t5)
...
#         origin_sorted_train_ids = torch.load(full_out_dir + '/' + args.model + '_' + method + '_removed_ids_v' + str(iter_count))
#         
#         ordered_list = torch.load(full_out_dir + '/' + args.model + '_' + method + '_removed_ids_weight_v' + str(iter_count))
#         
#         final_influence_values = torch.load(full_out_dir + '/' + args.model + '_' + method + '_v' + str(iter_count))
        
#         s_test_vec = torch.load(full_out_dir + '/' + args.model + '_' + method + '_s_test_vec_v' + str(iter_count))
        
        
        torch.save(origin_sorted_train_ids_2, full_out_dir + '/' + args.model + '_' + method + '_removed_ids_v' + str(iter_count) + incremental_suffix)
    
        torch.save(ordered_list_2, full_out_dir + '/' + args.model + '_' + method + '_removed_ids_weight_v' + str(iter_count) + incremental_suffix)
        
        torch.save(final_influence_values_2, full_out_dir + '/' + args.model + '_' + method + '_v' + str(iter_count) + incremental_suffix)
        
#         torch.save(s_test_vec_2, full_out_dir + '/' + args.model + '_' + method + '_s_test_vec_v' + str(iter_count) + incremental_suffix)
        
        print('here')
    

    updated_labels, full_existing_labeled_id_tensor = obtain_updated_labels(full_out_dir, args, full_training_noisy_dataset.data, full_training_noisy_dataset.labels, full_training_origin_labels, existing_labeled_id_tensor=full_existing_labeled_id_tensor, size = None, start = False, iter_count = iter_count, is_incremental = is_incremental, derive_probab_labels = derive_probab_labels, method = method)
    ...
    probs = final_influence_values/(torch.sum(final_influence_values))

    average_y_change = 1.0/(torch.sum(ids_with_changed_ids).item()*updated_labels.shape[0]*1.0)*torch.sum((updated_labels.view(-1) - full_training_noisy_dataset.labels.view(-1))[ids_with_changed_ids]/probs[ids_with_changed_ids].view(-1)).type(torch.double).item()


    return updated_labels, full_existing_labeled_id_tensor, ids_with_changed_ids, ids_with_unchanged_ids, average_y_change


def model_training_main(model, optimizer, random_ids_multi_super_iterations, w_list, grad_list, full_training_noisy_dataset, updated_labels, args, validation_dataset, dataset_test, is_incremental, iter_count, ids_with_changed_ids, ids_with_unchanged_ids, full_existing_labeled_id_tensor):
...
def load_retraining_influence_info(iter_count, full_out_dir, args, method = 'ac'):
    sorted_train_ids = torch.load(full_out_dir + '/' + args.model + '_' + method + '_removed_ids_v' + str(iter_count) + incremental_suffix)
        
    ordered_list = torch.load(full_out_dir + '/' + args.model + '_' + method + '_removed_ids_weight_v' + str(iter_count) + incremental_suffix)

    influences1 = torch.load(full_out_dir + '/' + args.model + '_' + method + '_v' + str(iter_count) + incremental_suffix)    

    exp_sorted_train_ids = torch.load(full_out_dir + '/' + args.model + '_' + method + '_removed_ids_v' + str(iter_count))
        
    exp_ordered_list = torch.load(full_out_dir + '/' + args.model + '_' + method + '_removed_ids_weight_v' + str(iter_count))

    influences2 = torch.load(full_out_dir + '/' + args.model + '_' + method + '_v' + str(iter_count))
...
#     training_dataset, val_dataset, test_dataset, full_output_dir = obtain_chexpert_examples(args)
    ...
    full_training_noisy_dataset.labels = full_training_noisy_dataset.labels[:,1]
    
#     full_training_origin_labels = full_training_origin_labels[:,1]
...
#         most_influence_point, ordered_list, sorted_train_ids = evaluate_influence_function_repetitive2(args, full_training_noisy_dataset, full_training_origin_labels, validation_dataset, dataset_test, model, 0, model.soft_loss_function, args.model, args.num_class)
        optimizer = model.get_optimizer(args.tlr, args.wd)
        updated_labels, full_existing_labeled_id_tensor, ids_with_changed_ids, ids_with_unchanged_ids, average_y_change = select_samples_influence_function_main(args, full_training_noisy_dataset, full_training_origin_labels, validation_dataset, dataset_test, model, full_out_dir, False, 0, None, w_list, grad_list, derive_probab_labels = False)
...
#         w_diff_remove_1, c2, mu = pre_compute_influence_order_history(exp_w_list, exp_grad_list, model, full_training_noisy_dataset, model.soft_loss_function_reduce, optimizer, random_ids_multi_super_iterations, args.bz, args.wd)
        ...
        torch.save(average_y_change, full_out_dir + '/average_y_change_v0')
...
#         iter_count = 9
        ...
        if args.incremental and iter_count > 0:
            average_y_change = torch.load(full_out_dir + '/average_y_change_v' + str(iter_count) + incremental_suffix)
        else:
            average_y_change = torch.load(full_out_dir + '/average_y_change_v' + str(iter_count))
        ...
        updated_labels, full_existing_labeled_id_tensor, ids_with_changed_ids, ids_with_unchanged_ids, average_y_change = select_samples_influence_function_main(args, full_training_noisy_dataset, full_training_origin_labels, validation_dataset, dataset_test, model, full_out_dir, is_incremental, iter_count + 1, full_existing_labeled_id_tensor,w_list, grad_list, derive_probab_labels = False, average_y_change = average_y_change)
        
        
        
#         if continue_labeling:

        model_training_main(model, optimizer, random_ids_multi_super_iterations, w_list, grad_list, full_training_noisy_dataset, updated_labels, args, validation_dataset, dataset_test, is_incremental, iter_count + 1, ids_with_changed_ids, ids_with_unchanged_ids, full_existing_labeled_id_tensor)
        ...
        if not is_incremental:
            ...
            torch.save(average_y_change, full_out_dir + '/average_y_change_v' + str(iter_count + 1))
...
#             torch.save(model, full_out_dir + '/model_v' + str(iter_count+1) + incremental_suffix)
            ...
            torch.save(average_y_change, full_out_dir + '/average_y_change_v' + str(iter_count + 1) + incremental_suffix)
