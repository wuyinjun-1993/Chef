...
class FeedforwardFlexible(torch.nn.Module):
    def __init__(self, h_sizes, activations, num_class = 1):
        ...
        if num_class == 1:
            ...
            self.layers.append(torch.nn.Linear(h_sizes[-1], num_class))
...
def soft_loss_function(logit, targets):
    loss_val = -(torch.sum(torch.log(logit) * targets, dim=1))
    
    return loss_val

class TorchMLP:
    def __init__(self, h_sizes=[150, 10, 10], activations=[torch.nn.ReLU(), torch.nn.ReLU()], optimizer='Adam',
                 optimparams={}, nepochs=200, num_class =1):
        self.model = FeedforwardFlexible(h_sizes, activations, num_class = num_class).float()
