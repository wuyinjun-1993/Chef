...
def train_end_classifier(Xtrain, Xtest, iwssession, lfsets, device='cpu', gap=20,
...
def train_end_classifier_with_neutral(Xtrain, Xtest, iwssession, lfsets, device='cpu', gap=20,
                         class_balance=None, modelparams=None, uniform=False, verbose=False, num_class = 2, label_mappings = {1:1, -1:-1}):
    """
    Function to fit label model, train downstream classifier, and return test set predictions
        ...
        for iteration_idx in iters_to_run:
            ...
            if uniform:
                ...
                for label in label_mappings.keys():
                    Lambda_snorkel[Lambda_snorkel == label] = label_mappings[label]
                # Lambda_snorkel[Lambda_snorkel == -1] = 0
                Lambda_snorkel[Lambda_snorkel == -10] = -1
                ...
                if 'cuda' in device:
                    torch.cuda.empty_cache()
                    label_model = LabelModel(cardinality=num_class, verbose=True, device=device)
                    label_model.fit(Lambda_snorkel[filteridx], class_balance=class_balance)
                    torch.cuda.empty_cache()
                else:
                    label_model = LabelModel(cardinality=num_class, verbose=True)
                    label_model.fit(Lambda_snorkel[filteridx], class_balance=class_balance)

                # get label estimate
                posterior = label_model.predict_proba(Lambda_snorkel)
                bin_posterior = posterior.astype(np.float32)
            ...
            torch.cuda.empty_cache()
            model = TorchMLP(h_sizes=modelparams['h_sizes'], activations=modelparams['activations'],
                             optimizer='Adam', nepochs=250, num_class = num_class)
